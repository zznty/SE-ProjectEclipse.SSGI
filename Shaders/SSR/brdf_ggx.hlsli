...
float3 fresnel_schlick(float3 f0, float f90, float v_dot_h)
{
    ...
    float pow5 = pow(max(0, 1.0 - v_dot_h), 5);
...
float3 fresnel(float metalness, float3 baseColor, float3 viewDir, float3 lightDir)
{
    float3 h = normalize(viewDir + lightDir);
    float v_dot_h = dot(viewDir, h); // same as l_dot_h
...
float3 BrdfSpecular(float roughness, float metalness, float3 baseColor, float3 viewDir, float3 lightDir, float3 normal)
{
    float a = roughness * roughness;
    float3 h = normalize(viewDir + lightDir);
...
float3 BrdfDiffuse(float roughness, float metalness, float3 baseColor, float3 viewDir, float3 lightDir, float3 normal)
{
    float a = roughness * roughness;
    float3 h = normalize(viewDir + lightDir);
    float n_dot_h = dot(normal, h);
    ...
    bool valid = n_dot_l > 0 && n_dot_v > 0;
    return ((1.0 - f) * d) * valid;
}

float BrdfSpecularNoFresnel(float roughness, float metalness, float3 baseColor, float3 viewDir, float3 lightDir, float3 normal)
{
    float a = roughness * roughness;
    float3 h = normalize(viewDir + lightDir);
    float n_dot_h = dot(normal, h);

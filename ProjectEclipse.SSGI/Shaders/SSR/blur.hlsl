...
float4 MipBlur(const float4 position : SV_Position, const float2 uv : TEXCOORD0) : SV_Target0
{
    ...
    for (int i = 0; i < numSamples; i++)
    {
        ...
        float4 sampleColor = sourceTex.SampleLevel(LinearSampler, uv + offset, MipLevel);
